...
Para executar este script faça:
                               $ streamlit run chunkerizer.py 


Este maravilho projeto é de "Gustavo Espíndola". 
"""
...
# Tokenize (OpenAI)
# Este código cria um objeto de codificação (Encoding) de Token para o esquema de codificação (Encoding) CL100K_BASE.
# The CL100K_BASE encoding scheme is used to encode tokens for the CL100K dataset.
import tiktoken
...
def create_dataframe(text_splitter, file_content):
  ...
  if len(df) < 1:
    st.error('Nenhum conteúdo encontrado.')
  else:
...
st.markdown("""#### Usando [@langchain](https://python.langchain.com/docs/modules/data_connection/document_transformers/) text splitter para chunk de code e arquivos de text.""")
st.divider()
...
uploaded_file=st.sidebar.file_uploader(
  "Solte um arquivo aqui ou clique para fazer upload",
...
if uploaded_file is not None:
  ...
  with st.expander(f"Ver arquivo original {uploaded_file.name} – {st.session_state['file_language']}"):
    st.code(file_content, language=f"{st.session_state['file_language']}", line_numbers=True)

#Credits
st.sidebar.markdown("Este projeto foi criado por [Gustavo Espíndola](https://github.com/gustavoespindola)")